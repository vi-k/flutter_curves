...
const IMap<String, Curve> _curvesTemplatesMap = IMapConst({
...
IList<String>? _curvesTemplates;
IList<String> get curvesTemplates =>
    _curvesTemplates ??= _curvesTemplatesMap.keys.toIList();

bool isElasticCurve(Curve curve) =>
    curve is ElasticInCurve ||
    curve is ElasticOutCurve ||
    curve is ElasticInOutCurve;

Curve? findCurveByName(String name) => _curvesTemplatesMap[name];

String? findNameByCurve(Curve curve) {
  for (final entry in _curvesTemplatesMap.entries) {
