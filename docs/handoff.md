...
**Публикуется с ветки `gh-pages`, вручную.** Пуш в `main` сайт не меняет.
Деплой — две команды из корня:

```sh
flutter build web --wasm --base-href /
# затем содержимое build/web выложить в ветку gh-pages
```

**Выложенная сборка отстаёт от `main`:** правка `DialogForPageRoute` от
2026-09-12 на сайт не попадала. Деплой ручной, и решение о нём — за
владельцем.
...
Домен приходит из `web/CNAME`: Flutter копирует `web/` в сборку, а
публикация с ветки читает `CNAME` из выложенного дерева. Держать его
только на `gh-pages` нельзя — первый же деплой снёс бы домен.

**CI здесь пока невозможен.** Workflow снял бы ручную выкладку, но
`auto_scroll_band` подключён по пути к **приватному** репозиторию, а
`GITHUB_TOKEN` внутри Actions видит только свой. Три развязки и их цена —
в `docs/records/2026-09-12[4]-pages-deploy-report.md`; выбор за
владельцем.
