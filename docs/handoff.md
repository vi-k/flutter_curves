...
### Ветка `gh-pages` мертва

Первый прогон workflow прошёл целиком, и `build`, и `deploy`. Переключать
Settings → Pages → Source руками не понадобилось: GitHub отдаёт последний
деплой, а он теперь приходит из Actions.

Что сайт идёт именно оттуда, видно по расхождению:

```
ветка gh-pages:   707374a, flutter_bootstrap.js 13 295 байт
отдаётся живьём:  13 211 байт
```

Ветка осталась на ручной сборке и больше не обновляется. **Её стоит
удалить** — иначе через полгода никто не скажет, откуда берётся сайт.

Отдельно: `gh api repos/vi-k/flutter_curves/pages` до сих пор отвечает
`"build_type": "legacy"` и `"branch": "gh-pages"`. Поле о живом сайте не
говорит ничего; верить надо тому, что отдаётся.
