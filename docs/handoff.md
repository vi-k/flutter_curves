...
### Чего не хватает, чтобы это заработало

Workflow лежит в дереве, но деплой из него пока невозможен:
**Settings → Pages → Source всё ещё «Deploy from a branch», `gh-pages`.**
Пока не переключено на «GitHub Actions», job `deploy` будет падать, а
сайт продолжит отдаваться со старой ветки.

Порядок: запушить `main`, переключить источник, прогнать workflow
(`workflow_dispatch`), проверить сайт. После этого ветка `gh-pages`
становится мёртвой — её стоит удалить, чтобы не гадать потом, откуда
берётся сайт.
